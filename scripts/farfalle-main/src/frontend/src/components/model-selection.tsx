...
export const modelMap: Record<ChatModel, Model> = {
  ...
  // Gemini Cloud Models
  // If backend exposes new enums via OpenAPI, regenerate; until then map via custom labels
  // We’ll piggyback existing cloud tab by extending type at runtime when enum updates land.
  // Placeholder entries will be ignored if enum lacks these keys.
  // @ts-ignore - guarded at runtime
  ["gemini-1.5-pro" as unknown as ChatModel]: {
    name: "Gemini 1.5",
    description: "Google/Gemini 1.5 Pro",
    value: "gemini-1.5-pro" as unknown as ChatModel,
    smallIcon: <SparklesIcon className="w-4 h-4 text-[#22C55E]" />,
    icon: <SparklesIcon className="w-5 h-5 text-[#22C55E]" />,
  },
  // @ts-ignore
  ["gemini-2.5-pro" as unknown as ChatModel]: {
    name: "Gemini 2.5",
    description: "Google/Gemini 2.5 Pro",
    value: "gemini-2.5-pro" as unknown as ChatModel,
    smallIcon: <SparklesIcon className="w-4 h-4 text-[#16A34A]" />,
    icon: <SparklesIcon className="w-5 h-5 text-[#16A34A]" />,
  },
  [ChatModel.LLAMA3]: {
    name: "Llama3",
