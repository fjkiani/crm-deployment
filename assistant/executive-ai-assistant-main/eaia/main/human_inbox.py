...
from eaia.crm_bridge import post_draft_with_provider, send_communication
...
async def send_email_draft(state: State, config, store):
    ...
    if response["type"] == "response":
        ...
        if memory:
            await save_email(state, config, store, "email")
        # On acceptance, create Draft in CRM using current tool call args (HTML/content)
        try:
            args = tool_call["args"]
            ...
            await post_draft_with_provider(email_payload)
        except Exception:
            pass
        return None
    else:
