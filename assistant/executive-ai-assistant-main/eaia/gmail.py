...
import email.utils
try:
    from langchain_auth import Client as LCAuthClient
except Exception:
    LCAuthClient = None
...
async def get_credentials(
    ...
    Order of preference:
    1) Local token file at eaia/.secrets/token.json (direct OAuth flow)
    2) LangChain Auth if available and configured
    """
    # 1) Direct local token file
    secrets_dir = Path(__file__).resolve().parent / ".secrets"
    # when running from repo structure, .secrets is at eaia/.secrets
    if not secrets_dir.exists():
        secrets_dir = Path(__file__).resolve().parents[1] / "eaia" / ".secrets"
    token_path = secrets_dir / "token.json"
    client_secrets = secrets_dir / "secrets.json"
    ...
    # If no token and we have client secrets, instruct user to run setup script
    if client_secrets.exists():
        raise RuntimeError(
            "Google tokens not found. Run: python scripts/setup_gmail_direct.py to authorize Gmail."
        )

    # 2) LangChain Auth fallback (if available)
    api_key = langsmith_api_key or os.getenv("LANGSMITH_API_KEY")
    if LCAuthClient and api_key:
        client = LCAuthClient(api_key=api_key)
        try:
            auth_result = await client.authenticate(
                provider="google", scopes=_SCOPES, user_id=user_email
            )
            if auth_result.needs_auth:
                ...
                completed_result = await client.wait_for_completion(
                    auth_id=auth_result.auth_id, timeout=300
                )
    ...
    raise RuntimeError(
        "No Gmail credentials available. Provide eaia/.secrets/secrets.json and run scripts/setup_gmail_direct.py"
    )
...
def create_message(sender, to, subject, message_text, thread_id, original_message_id):
    ...
    # Heuristic: send HTML if content appears to be HTML
    if isinstance(message_text, str) and "<" in message_text and ">" in message_text:
...
def send_email(
    ...
    # Send the response
    result = send_message(service, "me", response_message)
    return result
