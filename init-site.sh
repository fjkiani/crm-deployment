...
# 2. Run site setup in background with explicit logging
...
echo "gunicorn=$(which gunicorn 2>/dev/null || echo NOT_FOUND)" | tee -a "$LOG"
...
if kill -0 $GUNICORN_PID 2>/dev/null; then
    ...
    echo "ERROR: gunicorn died! Trying simpler config..." | tee -a "$LOG"
    /home/frappe/frappe-bench/env/bin/gunicorn \
        --chdir=/home/frappe/frappe-bench/sites \
        --bind=0.0.0.0:${SERVE_PORT} \
        --timeout=120 \
        frappe.app:application &
    GUNICORN_PID=$!
    echo "gunicorn retry PID=$GUNICORN_PID" | tee -a "$LOG"
    sleep 3
fi

# Run site setup in background with explicit PATH and logging
export SAVED_GUNICORN_PID=$GUNICORN_PID
export SAVED_BENCH_DIR=$BENCH_DIR
export SAVED_SITE=$SITE
export SAVED_DB_HOST=$DB_HOST
export SAVED_DB_PORT=$DB_PORT
export SAVED_DB_NAME=$DB_NAME
export SAVED_DB_PASSWORD=$DB_PASSWORD
export SAVED_ADMIN_PASSWORD=$ADMIN_PASSWORD
export SAVED_ENCRYPTION_KEY=$ENCRYPTION_KEY
export SAVED_LOG=$LOG

bash << 'BGSCRIPT' >> "$LOG" 2>&1 &
export PATH="/usr/local/bin:/home/frappe/.local/bin:/home/frappe/frappe-bench/env/bin:$PATH"
echo "[bg] Starting at $(date)"
echo "[bg] PATH=$PATH"
echo "[bg] bench=$(which bench 2>/dev/null || echo NOT_FOUND)"
echo "[bg] GUNICORN_PID=$SAVED_GUNICORN_PID"

BENCH_DIR="$SAVED_BENCH_DIR"
SITE="$SAVED_SITE"
...
kill -HUP $SAVED_GUNICORN_PID 2>/dev/null && echo "[bg] Sent SIGHUP to gunicorn" || echo "[bg] SIGHUP failed"
BGSCRIPT

echo "Background setup started (PID=$!)" | tee -a "$LOG"
